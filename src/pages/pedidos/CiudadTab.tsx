...
import { usePedidosConfig, useEnviarPedidoCiudad } from "./hooks";
...
export default function CiudadTab() {
  ...
  const mutation = useEnviarPedidoCiudad();

  const canSubmit = proveedor && ciudad && tipo && mes && anio && archivo && remitente;

  const handleSubmit = useCallback(() => {
    if (!proveedor || !ciudad || !tipo || !mes || !anio || !archivo || !remitente)
      return;
  ...
  }, [proveedor, ciudad, tipo, mes, anio, archivo, remitente, mutation, user]);
    ...
    <div className="space-y-5">
      {step === "config" && (
        <div className="space-y-5">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="remitente-ciudad">Remitente</Label>
            <Input
              id="remitente-ciudad"
              value={remitente}
