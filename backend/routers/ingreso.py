...
async def debug_sales_channels():
    ...
    try:
        ...
        steps = [{"debug_info": {"graphql_url": graphql_url, "token_present": bool(cfg.SHOPIFY_ACCESS_TOKEN)}}]
        ...
        create_mut = "mutation productCreate($product: ProductCreateInput!) { productCreate(product: $product) { product { id title variants(first: 1) { nodes { id } } } userErrors { field message } } }"
        create_resp = session.post(graphql_url, json={"query": create_mut, "variables": {"product": {"title": "PRUEBA SALES CHANNELS - ELIMINAR", "vendor": "TEST", "productType": "Libro", "status": "ACTIVE"}}}, timeout=30)
        ...
        # Paso 4: Publicar
        pub_mut = "mutation publishablePublish($id: ID!, $input: [PublicationInput!]!) { publishablePublish(id: $id, input: $input) { publishable { availablePublicationsCount { count } } userErrors { field message } } }"
